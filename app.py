...
def generate_quiz_data_batched(topic, subtopic, target_count, due_date, due_time, points_per_question, question_types, grade_level, is_sped, is_ml, language, source_text="", context_topics=None):
    ...
    for i in range(num_batches):
        ...
        prompt = construct_quiz_prompt(topic, subtopic, batch_size, due_date, due_time, points_per_question, question_types, grade_level, is_sped, is_ml, language, source_text, context_topics)
...
def generate_unit_package(sequence_data, topic, grade_level, is_sped, is_ml, language, subject, strategy, source_text, due_date, due_time, points, points_per_question, question_types):
    """Generates all files for a unit and zips them."""
    context_buffer = []
    zip_buffer = io.BytesIO()
    ...
    with zipfile.ZipFile(zip_buffer, "w", zipfile.ZIP_DEFLATED) as zf:
        for i, item in enumerate(sequence_data):
            ...
            status_text.text(f"Generating {item_type} {idx}/{total_items}: {title}...")
            
            if item_type == "Assignment":
                # Generate Assignment HTML
                # We need to construct the prompt first
                # Note: We use 'None' for tool in Unit mode for simplicity, or we could auto-detect. 
                # Let's use "None" for now as per plan implies focusing on the sequence.
                prompt = construct_assignment_prompt(topic, focus, "None", due_date, due_time, points, grade_level, is_sped, is_ml, language, subject, strategy, source_text)
                ...
                quiz_data = generate_quiz_data_batched(topic, focus, 10, due_date, due_time, points_per_question, question_types, grade_level, is_sped, is_ml, language, source_text, context_topics=context_buffer)
...
def construct_quiz_prompt(topic, subtopic, count, due_date, due_time, points_per_question, question_types, grade_level, is_sped, is_ml, language, source_text="", context_topics=None):
...
### 3. TASK
Create a logical unit sequence mixing {num_assignments} Assignments and {num_quizzes} Quizzes.
Instructional Strategy: {strategy}
Instruction: Place quizzes after relevant assignments to assess learning.
...
Example:
[
  {{"type": "Assignment", "title": "Intro to Cells", "focus_topic": "Cell Theory"}},
  {{"type": "Quiz", "title": "Cell Theory Check", "focus_topic": "Cell Theory"}}
]
"""
    return prompt
...
if content_type == "Assignment":
    ...
    if st.button("Draft My Mega-Prompt"):
        ...
        with st.spinner("Planning Unit Sequence..."):
            prompt = construct_unit_prompt(topic, num_assignments, num_quizzes, grade_level, is_sped, is_ml, language, subject, instructional_strategy, source_text)
            sequence_data = generate_unit_sequence_json(prompt)
            
        if sequence_data:
            # 2. Generate Package
            with st.spinner("Generating Unit Resources (This may take a moment)..."):
                unit_zip = generate_unit_package(sequence_data, topic, grade_level, is_sped, is_ml, language, subject, instructional_strategy, source_text, due_date, due_time, points, points_per_question, question_types)
