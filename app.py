...
def create_quiz_xml(questions, title="Generated Quiz"):
    ...
    for i, q in enumerate(questions):
        ...
        for j, option in enumerate(q.options):
            ...
            mattext_opt = ET.SubElement(material_opt, "mattext", {"texttype": "text/plain"})
            mattext_opt.text = option
...
def generate_lesson_plan_pdf(topic, standard, grade, strategy="None / Standard"):
    ...
    try:
        ...
        def sidebar_section(title, items, max_lines=10):
            nonlocal y_pos
            if y_pos > 260: return # Stop if too low
            ...
            # Estimate height used (rough)
            lines = safe_content.count('\n') + 1
            y_pos += (lines * 4) + 4
...
if content_type == "Assignment":
    ...
    if st.button("Draft My Mega-Prompt"):
        st.session_state['generated_prompt'] = construct_quiz_prompt(topic, subtopic, question_count, due_date, due_time, points_per_question, question_types, grade_level, is_sped, is_ml, language, source_text)
        st.session_state['is_generated'] = True
